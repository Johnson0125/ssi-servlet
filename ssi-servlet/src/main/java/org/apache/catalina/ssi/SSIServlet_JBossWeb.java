...
public class SSIServlet_JBossWeb  extends HttpServlet {
    ...
    // ssi-rest - erg - added method, so the buffered text can be processed with additional features
    ...
    protected void processSSI(HttpServletRequest req, HttpServletResponse res,
        ...
        if (buffered) {
            printWriter.flush();
            // ssi-rest - erg - added call to new method, so the buffered text can be processed with additional features
            String text = processBuffered(req, stringWriter.toString());
